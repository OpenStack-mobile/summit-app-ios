...
public class EventRemoteDataStore: NSObject {
    ...
    public func getFeedbackForEvent(eventId: Int, page: Int, objectsPerPage: Int, completionBlock : ([Feedback]?, NSError?) -> Void) {
        ...
        http.GET("https://openstackid-resources.openstack.org/api/v1/summits/current/events/\(eventId)/feedback?expand=owner&page=\(page)&per_page=\(objectsPerPage)") {(responseObject, error) in
            if (error != nil) {
